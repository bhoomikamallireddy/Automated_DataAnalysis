...
function getDistributionPlotConfig(chartType, sampleData) {
  switch (chartType) {
    case "violin":
      return {
        type: "violin",
...
export default function Home() {
  ...
  // Loading state when selecting from history
      ...
      <main className="flex-1 overflow-y-auto flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-zinc-200 px-3 md:px-8 py-3 flex justify-between items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 md:gap-3 min-w-0">
            ...
            <label
              htmlFor="csv-upload"
              className="px-2.5 py-1.5 text-[10px] font-bold bg-zinc-100 hover:bg-zinc-200 rounded-md cursor-pointer transition-all active:scale-95 border border-zinc-200 truncate max-w-[90px] sm:max-w-[140px] shrink-0"
            >
        ...
        <div className="p-4 md:p-8 w-full mx-auto space-y-8">
          ...
          {workspaceMode === "current" ? (
            <div className="w-full space-y-8">
              {/* Transition Loader for History -> Current */}
              {isSwitchingMode ? (
                <div className="py-20 text-center animate-pulse">
                  <div className="h-12 w-12 bg-zinc-200 rounded-full mx-auto mb-4"></div>
                  <div className="h-4 w-48 bg-zinc-200 mx-auto rounded"></div>
                </div>
              ) : (
                <>
                  {/* ZERO STATE */}
                  {!currentJobId && (
                    <div className="flex flex-col items-center justify-center py-24 md:py-32 text-center border-2 border-dashed border-zinc-200 rounded-3xl bg-white">
                      ...
                      <p className="text-zinc-500 mt-2 max-w-sm px-4">
                        Upload a CSV file in the header to trigger the Automated
                        EDA & Machine Learning engine.
                      </p>
                    </div>
                  )}
                    ...
                    <div className="animate-pulse bg-blue-600 text-white p-6 md:p-8 rounded-3xl flex justify-between items-center shadow-xl shadow-blue-100">
                      <div>
                        ...
                        <p className="text-sm opacity-80 font-medium">
                          Calculating PCA coordinates and semantic executive
                          summaries.
                        </p>
                      </div>
                  ...
                  {/* COMPLETED DASHBOARD */}
                  {jobStatus === STATUS_COMPLETED && results && (
                    <>
                      {/* LAYER 2: KPI RIBBON */}
                      <section className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                        <HealthGauge
                          score={results?.metadata?.health_score || 0}
                        />
                        <MetricCard
                          label="Total Rows"
                          value={
                            results?.metadata?.total_rows?.toLocaleString() ||
                            "0"
                          }
                        />
                        <MetricCard
                        ...
                        <MetricCard
                          label="Anomalies"
                          value={Object.values(results?.outliers || {}).reduce(
                            (a, b) => a + b,
                            0,
                          )}
                        ...
                        <div className="p-4 md:p-10 flex-1">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="p-6 border border-zinc-100 rounded-2xl bg-white shadow-sm hover:shadow-md transition-shadow">
                                  ...
                                  <p className="text-sm text-zinc-600 leading-relaxed">
                                    ...
                                    dimensions with a focus on variance-driven
                                    feature importance.
                                  </p>
                                </div>
                                ...
                                <div className="p-6 bg-purple-50/50 border border-purple-100 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
                                  ...
                                  <p className="text-sm text-purple-900 leading-relaxed">
                                    Unsupervised{" "}
                                    <span className="font-bold">PCA</span> has
                                    reduced the dataset into 2 primary
                                    components for structural visualization and
                                    clustering analysis.
                                  </p>
                                </div>
                              </div>
                                ...
                                <div className="lg:col-span-2 p-6 bg-white border border-zinc-100 rounded-3xl shadow-sm hover:shadow-md transition-shadow">
                                  <div className="flex justify-between items-start mb-4">
                                    <div>
                                      ...
                                      <p className="text-xs text-zinc-500">
                                        Dendrogram logic: Correlation between
                                        null values
                                      </p>
                                    </div>
                                  ...
                                  <div className="h-64 w-full">
                                    {results?.ml_insights?.quality_metrics
                                      ?.null_correlation?.labels?.length > 0 ? (
                                      <PlotlyChart
                                        data={[
                                          {
                                            z: results?.ml_insights
                                              ?.quality_metrics
                                              ?.null_correlation?.z,
                                            x: results?.ml_insights
                                              ?.quality_metrics
                                              ?.null_correlation?.labels,
                                            y: results?.ml_insights
                                              ?.quality_metrics
                                              ?.null_correlation?.labels,
                                            type: "heatmap",
                                            colorscale: "Purples",
                                      ...
                                      <div className="h-full flex flex-col items-center justify-center bg-zinc-50/50 rounded-2xl border border-dashed border-zinc-200">
                                        ...
                                        <p className="text-xs text-zinc-400 font-medium">
                                          No missing values detected in this
                                          dataset.
                                        </p>
                                      </div>
                                    )}
                                      ...
                                      <React.Fragment key={col}>
                                        ...
                                        >
                                          <td className="px-6 py-4 font-semibold text-zinc-800 flex items-center gap-2">
                                            <span
                                              className={`text-[8px] transition-transform ${expandedRow === col ? "rotate-90" : ""}`}
                                            >
                                              ▶
                                            </span>
                                            {col}
                                          ...
                                          <td className="px-6 py-4 text-zinc-500">
                                            {results?.metadata?.missing_values[
                                              col
                                            ] || 0}
                                          </td>
                                          <td className="px-6 py-4 text-zinc-500">
                            ...
                            <div className="animate-in fade-in slide-in-from-right-4 duration-700 space-y-10 w-full">
                              ...
                              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-3">
                                <div>
                                  ...
                                  <p className="text-xs text-zinc-500">
                                    Analysis of the most influential feature
                                    interactions
                                  </p>
                                </div>
                              ...
                              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                  ...
                                  <div className="h-80 w-full">
                                    <PlotlyChart
                                      data={[
                                        {
                                          z: results?.ml_insights
                                            ?.influential_correlations?.z,
                                          x: results?.ml_insights
                                            ?.influential_correlations?.x,
                                          y: results?.ml_insights
                                            ?.influential_correlations?.y,
                                          type: "heatmap",
                                          colorscale: "RdBu",
                                          zmin: -1,
                                          zmax: 1,
                                ...
                                {(() => {
                                  const gallery =
                                    results?.ml_insights?.bivariate_gallery ||
                                    [];
                                  const topPair =
                                    gallery.length > 0
                            ...
                            <div className="animate-in fade-in slide-in-from-right-4 duration-700 space-y-8">
                              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div>
                                  ...
                                  <p className="text-xs text-zinc-500">
                                    Univariate statistical spread and density
                                    estimation
                                  </p>
                                </div>
                                ...
                                <div className="relative inline-block">
                                  <select
                                    value={distChartType}
                                    onChange={(e) =>
                                      setDistChartType(e.target.value)
                                    }
                                  ...
                                  >
                                    {DISTRIBUTION_CHART_OPTIONS.map(
                                      (option) => (
                                        <option
                                          key={option.value}
                                          value={option.value}
                                  ...
                                  <div className="h-72">
                                    ...
                                    >
                                      <ScatterChart>
                                        ...
                                        />
                                        <Scatter
                                          data={results.ml_insights?.pca_data?.map(
                                            (p) => ({ x: p[0], y: p[1] }),
                                          )}
                                          fill="#8b5cf6"
                                          fillOpacity={0.4}
                          ...
                          {activeTab === "recommendations" && (
                            <div className="animate-in fade-in slide-in-from-right-4 duration-700 space-y-8">
                              ...
                              {/* FEATURE SUGGESTION CARD (ENGINEERING) */}
                    ...
                    <tbody className="divide-y divide-zinc-50">
                      {history.map((job) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <span
                              className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-tighter ${getStatusBadgeClass(
                                job.status,
                              )}`}
                            >
                              {job.status}
                          ...
                          <td className="px-6 py-4 text-right">
                            <button
                              type="button"
                              onClick={() =>
                                handleViewResultFromHistory(job.id)
                              }
